...
import '../lib/spaced_repetition.dart';
...
void main() {
  group('SpacedRepetitionScheduler', () {
    ...
    test('success advances interval', () {
      final item = sched.initial('harf_alif', nowUtc: baseNow);
      final next = sched.onReview(current: item, success: true, nowUtc: baseNow.add(const Duration(days: 1)));
    ...
    test('failure resets interval', () {
      final item = sched.initial('harf_alif', nowUtc: baseNow);
      final progressed = sched.onReview(current: item, success: true, nowUtc: baseNow.add(const Duration(days: 1)));
      final failed = sched.onReview(current: progressed, success: false, nowUtc: baseNow.add(const Duration(days: 2)));
